...
int main(int argc, char *argv[])
{
	...
	char buffer[1000000]; // Char buffer for storing the message string to be encrypted
	char buffer2[1000000]; // Char buffer for storing the key
	char completeBuffer[1000000]; // Char buffer for storing the response message from the encryption daemon 	
	char readBuffer[10]; // Char buffer for helping read in completeBuffer in chunks
	const char *validChar = "ABCDEFGHIJKLMNOPQRSTUVWXYZ ";	// List of valid letters for encryption	
	...
	if(buffer[strspn(buffer, validChar)] != '\0'){
		close(socketFD);
	...
	if(size1 > size2){fprintf(stderr, "Error: key '%s' is too short\n", buffer2); close(socketFD); exit(1);}else{
		// Format the buffer to be sent to the encryption daemon
		strcat(buffer, "@");
		strcat(buffer,"e");
		strcat(buffer, "@");
		...
		while(strstr(completeBuffer, "@@") == NULL){
			...
			if(strstr(completeBuffer, "e") != NULL){ // Error from connecting to the wrong daemon
				r = -2;
				break;
			}
			if(r == -1){ 
		...
		charsRead = r;
		if (charsRead == -2) error("ERROR: cannot connect to decrypt daemon\n");
